...
public class BrowserEmulator {
    ...
    */
	...
	String _capture(){
		return "screen to "+LogTools.screenShot(this);
	}
	/*
     *  click text by tag and exact text
     *  @return this
     *
	*/
	public BrowserEmulator clickText(String type,String text){
		try{
			getBrowserCore().findElementByXPath("//"+type+"[text()='"+text+"']").click();
		}
		catch(Exception ex)
		{
			throw new WebDriverException("text is "+text+" and tagname is  "+type+" cannot clickable,"+this._capture());
		}
		return this;
	}
	...
	public BrowserEmulator scrollPageTop()
	{
		getBrowserCore().executeScript("window.scrollTo(0,0)");
		return log("window scroll page To Right");
	}
	public BrowserEmulator scrollPageRight()
	{
		getBrowserCore().executeScript("window.scrollTo(document.body.scrollWidth,0)");
		return log("window scroll page To Right");
	}
	
	public BrowserEmulator scrollPageLowerRight()
	{
		getBrowserCore().executeScript("window.scrollTo(document.body.scrollWidth,document.body.scrollHeight)");
		return log("window scroll page To LowerRight");
	}
	...
	public BrowserEmulator wait4path(String path)
	{
		return wait4path(path,timeout);
	}
	...
	{
		try{
		 if(loc.startsWith(".")||loc.startsWith("#"))return browserCore.findElementByCssSelector(loc);
		 if(loc.startsWith("//")||!this.useCss)return browserCore.findElementByXPath(loc);
		 else return  browserCore.findElementByCssSelector(loc);
		}
		catch(Exception ex)
		{
			throw new NoSuchElementException("path "+loc+" not found,"+_capture());
		}
	}
	
	public List<WebElement> getElements(String loc)
	{
		try{
			if(!this.useCss)return browserCore.findElementsByXPath(loc);
			else return  browserCore.findElementsByCssSelector(loc);
		}
		catch(Exception ex)
		{
			throw new NoSuchElementException("path "+loc+" not found,"+_capture());
